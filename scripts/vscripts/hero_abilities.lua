...
function Interact( key )
	...
	for _,unit in pairs(units) do
		if unit:FindModifierByName('modifier_anvil') then
			local hAnvil = unit
			local hHammerAbility = caster:FindAbilityByName('anvil_hammer')
			-- hHammerAbility:SetChanneling(true)
			-- caster:CastAbilityImmediately(hHammerAbility, caster:GetPlayerID())
			caster:CastAbilityOnTarget(hAnvil, hHammerAbility, caster:GetPlayerID())
			-- caster:RemoveAbility('anvil_hammer')
			return
		end
	end
